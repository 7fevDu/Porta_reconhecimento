...
_SAMPLE_INTERVAL  = 0.4   # segundos entre cada amostra
...
def _draw_overlay(
    ...
    if state == "idle":
        ...
        status_text = f"Coletando... ({len(samples)}/{_SAMPLES_REQUIRED})"
        status_color = (0, 200, 255)
    else:
        name = final_user or "Desconhecido"
        verdict = "ACESSO CONCEDIDO" if state == "granted" else "ACESSO NEGADO"
        status_text = (
            f"{name}  |  Soma: {display_confidence:.0f} / {threshold_sum:.0f}  |  {verdict}"
        )
...
def run() -> None:
    ...
    # ── Estado da máquina ─────────────────────────────────────────────────────
    state: str                  = "idle"     # idle | collecting | granted | denied
    samples:      list[float]   = []
            ...
            while True:
                ...
                if state in ("granted", "denied"):
                    ...
                    if regions:
                        ...
                        samples.clear()
                        sample_users.clear()
                        last_sample_time = 0.0  # força 1ª coleta imediata
                    ...
                    if not regions:
                    ...
                    elif (now - last_sample_time) >= _SAMPLE_INTERVAL:
                        face = max(regions, key=lambda r: r.w * r.h)
                        user, conf = recognizer.identify(face.crop)
                        samples.append(conf)
                        ...
                        if len(samples) == _SAMPLES_REQUIRED:
                            # Soma das 3 amostras (máx 300). Libera se soma >= 90% × 3 = 270
                            ...
                            if (
                                ...
                                state = "denied"
                                log.info(
                                    f"ACESSO NEGADO — {final_user or 'desconhecido'} "
                                    f"| amostras={[f'{s:.1f}' for s in samples]} "
                                    f"| soma={final_confidence:.1f} / {threshold_sum:.0f}"
                                )
                ...
                _draw_overlay(frame, state, samples, display_conf, final_user)
